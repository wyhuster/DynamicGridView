package com.gtja.tonywang.dynamicgridview.widget;
...
import com.gtja.tonywang.dynamicgridview.adapter.DynamicGridAdapterInterface;
import com.gtja.tonywang.dynamicgridview.utils.DynamicGridUtils;
...
public class DynamicGridView extends GridView {
	...
	private static final int TAG_KEY = 2121034112;
	 ...
	 * {@link android.widget.AdapterView.OnItemClickListener} or
	...
	private void startWobbleAnimation() {
		for (int i = 0; i < getChildCount(); i++) {
			View v = getChildAt(i);
			if (v != null && Boolean.TRUE != v.getTag(TAG_KEY)) {
				if (i % 2 == 0)
					animateWobble(v);
				else
	...
	private void reorderElements(int originalPosition, int targetPosition) {
		if (mDragListener != null)
			mDragListener.onDragPositionsChanged(originalPosition,
					targetPosition);
		Log.i("wy", "reorder " + originalPosition + " " + targetPosition);
	...
	/**
	 * 清空idList，并且添加除了id为itemId的其他所有可重排序的item id到idList中
	 */
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		case MotionEvent.ACTION_DOWN:
			...
			// 返回第一个触摸电点的标志，pointerIndex从0到getPointerCount-1
			mActivePointerId = event.getPointerId(0);
			// 在edit模式下，edit模式长按激活
			if (mIsEditMode && isEnabled()) {
				// 父类GridView中的方法，为子项布局
				layoutChildren();
				// 父类AbsListView中的方法，根据坐标找到触摸点的位置
	...
	private void startDragAtPosition(int position) {
		...
		if (selectedView != null) {
			...
			// 返回一个跟当前item view一样的BitmapDrawable对象，用于在移动过程中显示
			mHoverCell = getAndAddHoverView(selectedView);
			// TODO 在移动过程中设置被移动的原位置view不可见
			if (isPostHoneycomb())
	...
	private void handleCellSwitch() {
		...
		for (Long id : idList) {
			...
			if (view != null) {
				...
				if ((aboveRight(targetColumnRowPair, mobileColumnRowPair)
						...
						&& deltaXTotal < view.getRight()
								- mOverlapIfSwitchStraightLine)) {
					float xDiff = Math.abs(DynamicGridUtils.getViewX(view)
							- DynamicGridUtils.getViewX(mMobileView));
					float yDiff = Math.abs(DynamicGridUtils.getViewY(view)
							- DynamicGridUtils.getViewY(mMobileView));
		...
		if (targetView != null) {
			...
			Log.i("wy", "handleCellSwitch " + originalPosition + " "
					+ targetPosition);
	...
	private OnScrollListener mScrollListener = new OnScrollListener() {
			...
			for (int i = 0; i < visibleItemCount; i++) {
				...
				if (child != null) {
					if (mMobileItemId != INVALID_ID
							&& Boolean.TRUE != child.getTag(TAG_KEY)) {
						if (i % 2 == 0)
							animateWobble(child);
						else
							animateWobbleInverse(child);
						child.setTag(TAG_KEY, true);
					} else if (mMobileItemId == INVALID_ID
							&& child.getRotation() != 0) {
						child.setRotation(0);
						child.setTag(TAG_KEY, false);
					}
				}
		...
		public void checkAndHandleFirstVisibleCellChange() {
			if (mCurrentFirstVisibleItem != mPreviousFirstVisibleItem) {
				if (mCellIsMobile && mMobileItemId != INVALID_ID) {
					updateNeighborViewsForId(mMobileItemId);
					Log.i("wy", "checkAndHandleFirstVisibleCellChange");
					handleCellSwitch();
				}
			}
		}
		...
		public void checkAndHandleLastVisibleCellChange() {
			...
			if (currentLastVisibleItem != previousLastVisibleItem) {
				if (mCellIsMobile && mMobileItemId != INVALID_ID) {
					updateNeighborViewsForId(mMobileItemId);
					Log.i("wy", "checkAndHandleLastVisibleCellChange");
					handleCellSwitch();
				}
			}
		}
	...
	private void startShakeAnimation() {
		Log.i("wy", "child count start:" + getChildCount());
		for (int i = 0; i < getChildCount(); i++) {
			View v = getChildAt(i);
			if (v != null && Boolean.TRUE != v.getTag(TAG_KEY)) {
				Animation rotateAnimation1 = new RotateAnimation(-2f, 2f,
				...
				v.startAnimation(rotateAnimation1);
				// animations.add(rotateAnimation1);
				v.setTag(TAG_KEY, true);
			}
		}
	}
	...
	private void stopShakeAnimation() {
		...
		Log.i("wy", "child count stop:" + getChildCount());
		for (int i = 0; i < getChildCount(); i++) {
			View v = getChildAt(i);
			if (v != null) {
				v.clearAnimation();
		...
		/*
		 * for (Animation animator : animations) { animator.cancel(); }
		 * animations.clear();
		 */
	}
}
